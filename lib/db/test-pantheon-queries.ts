...
async function testContinentFilter() {
  ...
  try {
    ...
    const europeResults = await db
      .select({ id: pantheonPerson.id, continent: pantheonPerson.birthplaceContinent })
      .from(pantheonPerson)
      .where(
        and(
...
async function testDomainFilter() {
  ...
  try {
    ...
    const artsResults = await db
      .select({ id: pantheonPerson.id, domain: pantheonPerson.domain })
      .from(pantheonPerson)
      .where(
        and(
          isNotNull(pantheonPerson.birthplaceLat),
...
async function testEraFilter() {
  ...
  try {
    ...
    const modernResults = await db
      .select({ id: pantheonPerson.id, era: pantheonPerson.era, birthyear: pantheonPerson.birthyear })
      .from(pantheonPerson)
      .where(
        and(
          isNotNull(pantheonPerson.birthplaceLat),
    ...
    logTest({
      ...
      details: modernResults.slice(0, 3).map((r) => ({ era: r.era, birthyear: r.birthyear })),
    });
    ...
    logTest({
      ...
      details: ancientResults.slice(0, 3).map((r) => ({ era: r.era, birthyear: r.birthyear })),
    });
  } catch (error) {
    logTest({
      name: 'Era filter',
...
async function testCountryFilter() {
  ...
  try {
    ...
    const usResults = await db
      .select({ id: pantheonPerson.id, country: pantheonPerson.birthplaceCountry })
      .from(pantheonPerson)
      .where(
        and(
          isNotNull(pantheonPerson.birthplaceLat),
    ...
    const validCountries = multiCountryResults.every(
      (r) =>
        r.country === 'United States' || r.country === 'United Kingdom' || r.country === 'France'
    );
...
async function testHPIFilter() {
  ...
  try {
    ...
    const highHPIResults = await db
      .select({ id: pantheonPerson.id, hpi: pantheonPerson.hpi, name: pantheonPerson.name })
      .from(pantheonPerson)
      .where(
        and(
          isNotNull(pantheonPerson.birthplaceLat),
    ...
    logTest({
      ...
      details: highHPIResults.slice(0, 5).map((r) => ({ name: r.name, hpi: r.hpi })),
    });

    // Test HPI range (50-70)
    const rangeResults = await db
      .select({ id: pantheonPerson.id, hpi: pantheonPerson.hpi })
      .from(pantheonPerson)
      .where(
        and(
          isNotNull(pantheonPerson.birthplaceLat),
...
async function testGenderFilter() {
  ...
  try {
    ...
    const maleResults = await db
      .select({ id: pantheonPerson.id, gender: pantheonPerson.gender })
      .from(pantheonPerson)
      .where(
        and(
          isNotNull(pantheonPerson.birthplaceLat),
...
async function testAliveFilter() {
  ...
  try {
    ...
    const aliveResults = await db
      .select({ id: pantheonPerson.id, alive: pantheonPerson.alive, name: pantheonPerson.name })
      .from(pantheonPerson)
      .where(
        and(
          isNotNull(pantheonPerson.birthplaceLat),
    ...
    logTest({
      ...
      details: aliveResults.slice(0, 5).map((r) => ({ name: r.name, alive: r.alive })),
    });
  } catch (error) {
    logTest({
      name: 'Alive filter',
    ...
    logTest({
      ...
      message:
        complexResults.length > 0
          ? 'Successfully applied complex filters'
          : 'No results (may be valid if criteria is very restrictive)',
      details:
        complexResults.length > 0
          ? complexResults.slice(0, 3).map((r) => ({
              name: r.name,
              hpi: r.hpi,
              domain: r.domain,
              alive: r.alive,
