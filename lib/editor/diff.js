...
export const computeChildEqualityFactor = (node1, node2) => 0;
...
export const matchNodeType = (node1, node2) => node1.type?.name === node2.type?.name
...
export const createDiffNode = (schema, node, type) => mapDocumentNode(node, node => {
  if (isTextNode(node)) {
